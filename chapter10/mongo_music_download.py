...
from chapter10.database.mongo_conn import col_song
from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor
from chapter10.config import ROOT_PATH
...
def get_singer_songs(singer_mid):
    ...
    for page_num in range(page_count):
        ...
        for i in music_data:
            ...
            val_dict = {"song_name": song_name, "song_ablum": song_ablum,
                        "song_interval": song_interval, "song_songmid": song_songmid,
                        "song_singer": song_singer}
            # 数据持久化到 MongoDB
            col_song().insert_one(val_dict)
            down_num += 1
            # 为演示使用，此处示例下载5首歌曲
            if down_num > 5:
                break
